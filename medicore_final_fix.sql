...
-- 'condition' is a reserved word — using backticks to escape it
-- ============================================================
...
-- ============================================================
-- FIX 2: All remaining tables not yet created
-- ============================================================
...
-- ============================================================
-- FIX 3: Indexes — MySQL does NOT support IF NOT EXISTS
-- for CREATE INDEX, so we use a safe stored procedure
-- ============================================================
...
CALL add_index_if_not_exists('patients',     'idx_patients_uhid',      'uhid');
CALL add_index_if_not_exists('patients',     'idx_patients_phone',     'phone');
CALL add_index_if_not_exists('appointments', 'idx_appt_date',          'appointment_date');
CALL add_index_if_not_exists('appointments', 'idx_appt_doctor',        'doctor_id');
CALL add_index_if_not_exists('appointments', 'idx_appt_patient',       'patient_id');
CALL add_index_if_not_exists('invoices',     'idx_invoices_patient',   'patient_id');
CALL add_index_if_not_exists('audit_logs',   'idx_audit_user',         'user_id');
CALL add_index_if_not_exists('notifications','idx_notifications_user', 'user_id');
...
INSERT IGNORE INTO notification_templates (name, notif_type, subject, body) VALUES
('appointment_confirmed', 'sms',   NULL, 'Dear {{patient_name}}, your appointment with Dr. {{doctor_name}} is confirmed on {{date}} at {{time}}. Token: {{token}}. MediCore HMS'),
('appointment_reminder',  'whatsapp', NULL, 'Reminder: Your appointment at MediCore is tomorrow {{date}} at {{time}} with Dr. {{doctor_name}}.'),
('lab_report_ready',      'sms',   NULL, 'Dear {{patient_name}}, your lab report for {{test_name}} is ready. MediCore HMS'),
('invoice_generated',     'email', 'Invoice #{{invoice_no}} - MediCore Hospital', 'Dear {{patient_name}}, your invoice #{{invoice_no}} for Rs.{{amount}} is ready.'),
('password_reset',        'email', 'Reset your MediCore HMS password', 'Dear {{name}}, reset your password here: {{reset_link}}. Expires in 1 hour.');

SET FOREIGN_KEY_CHECKS = 1;

-- ============================================================
-- Verify — shows all tables created
-- ============================================================
...
SELECT 'All done! MediCore HMS database is ready.' AS status;
